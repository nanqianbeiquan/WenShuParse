...
public class ParseUtil {
    ...
    public static Double parseMoneyType(String data){
        ...
        if(str.contains("万")){
            ...
            if(!str.replaceAll(regEx,"").equals(".") && !str.replaceAll(regEx,"").equals("")) {
                result = Double.parseDouble(String.format("%.6f",Double.parseDouble(str.replaceAll(regEx, "")) / 10000));
            }
        }
        return result;
    }
    public static String parseMoney(String data,String yuangao,String beigao) {
        String ws="";
      /*  if(data.indexOf("）")!=data.lastIndexOf("）")){
        ...
        String regMoney="((本金|借款|欠款|款项|租金|利息|复利|违约金|孳息|罚息|货款|保证金)+(人民币)?[0-9.]+(，|,)?[0-9.，,]*(，|,)?[0-9]*(万元|万|元)?)(按)?(以)?(月利率)?(年息)?(年)?(计算)?(的)?(利息)?(为基数)?(还清之日)?(基数)?(参照)?|(合计)?(诉讼费)?(受理费)?(上诉费)?(用)?([0-9.，]+(，)?[0-9.]*(金)?(元|万元|亿元|整))(元)?(计算)?(的)?(利息)?(为基数)?(为)?(借款)?(本金)?(计付)?|([0-9]+%.{0,5}[0-9.]+(元))(为基准)?|(一|二|三|四|五|六|七|八|九)+分之(一|二|三|四|五|六|七|八|九)+.{0,5}[0-9.]+(万)?元(?!为基数)";
                    ...
                    if(m.find()){
                        ...
                        if(m.group().indexOf("由") != m.group().lastIndexOf("由")){
                            ...
                            for(int z=0;z<crr.length;z++){
                                ...
                                String datapj=crr[z].replaceAll("[^0-9\\.]","");
                                String datapjresult=dealWithMoney(datapj);
                                if(matcherpersoncrr.find() && !crr[z].contains("受理费") && !datapjresult.equals("") && Double.parseDouble(datapjresult)>Double.parseDouble(maxmoney)){
                                    maxmoney=dealWithMoney(crr[z].replaceAll("[^0-9\\.]",""));
                                    if(maxmoney.equals("")){
                                        maxmoney="0";
                                    }
                                    maxperon=peoplecrr;
                            ...
                            if(m.group().indexOf(",")>=0 || m.group().indexOf("，")>=0){
                                ...
                                if(!maxperon.equals("") && maxperon.contains(yuangao)){
                                    fail = "原告";
                                }else if(!maxperon.equals("") && beigao.contains(maxperon)){
                                    fail = "被告";
                                ...
                                if (people.indexOf("被告")>= 0) {
                                    ...
                                    if (beigao.indexOf(people)>=0) {
                                        fail = "被告";
                                    } else if(yuangao.indexOf(people)>=0) {
                                        fail = "原告";
                                    }
                                }
                            }
        ...
        return Double.parseDouble(String.format("%.6f",totalMoney)) + "_" + fail;
    }
}
